...
const PopularRepoComponent = ({ repo }: PopularRepoProps) => {
  return (
    <div className="flex flex-col justify-between gap-2 w-full lg:max-w-[540px] h-[120px] border-[1px] border-gray-300 p-4 shadow-sm">
      <div className="flex flex-row  justify-between">
        <p className="text-[14px] font-semibold text-blue-700 hover:underline line-clamp-2">{repo.name}</p>
        <p className="bg-gray-200 px-2 py-1 rounded-2xl text-[12px] self-center">{repo.visibility}</p>
      </div>
      <div>
        <p className="text-[12px] truncate">{repo.description}</p>
      </div>
      <div>
